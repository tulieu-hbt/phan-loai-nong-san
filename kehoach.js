...
// Hàm hiển thị thông tin giới thiệu dưới dạng văn bản
function displayIntroductionText(gioiThieu, container) {
...
// Hàm hiển thị kế hoạch trồng cây dưới dạng văn bản
function displayPlantingPlanText(plantingPlan, container) {
    ...
    plantingPlan.forEach(task => {
        tasksHTML += `<p><strong>${task.STT || ""}:</strong> ${task['Cong Viec Can Lam'] || ""}</p>
                      <p><strong>Thời gian:</strong> ${task['Thoi Gian Thuc Hien'] || ""}</p>
                      <p><strong>Vật liệu:</strong> ${task['Vat Lieu, Dung Cu Can Thiet'] || ""}</p>
...
// Hàm hiển thị chi phí trồng cây dưới dạng văn bản
function displayCostEstimateText(costEstimate, container) {
    if (!Array.isArray(costEstimate)) {
        console.error("Dữ liệu chi phí trồng cây không hợp lệ");
        return;
    }
    ...
    costEstimate.forEach(item => {
        ...
        totalCost += itemTotal;
        costHTML += `<p><strong>${item.STT || ""}:</strong> ${item['Cac Loai Chi Phi'] || ""}</p>
                     <p><strong>Đơn vị:</strong> ${item['Don Vi Tinh'] || ""}</p>
                     <p><strong>Đơn giá:</strong> ${item['Don Gia (dong)'] || ""}</p>
                     <p><strong>Số lượng:</strong> ${item['So Luong'] || ""}</p>
                     <p><strong>Thành tiền:</strong> ${itemTotal}</p>
                     <p><strong>Ghi chú:</strong> ${item['Ghi Chu'] || ""}</p>`;
    });

    costHTML += `<p><strong>Tổng cộng:</strong> ${totalCost}</p>`;
    container.innerHTML = costHTML;
}

// Hàm hiển thị kế hoạch trồng cây dưới dạng bảng
function displayPlantingPlanTable(plantingPlan, container) {
    if (!container) {
...
// Hàm hiển thị chi phí trồng cây dưới dạng bảng
function displayCostEstimateTable(costEstimate, container) {
    if (!Array.isArray(costEstimate)) {
